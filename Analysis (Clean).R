...
# for bayes
accuracy_implicit <- as.data.frame(cbind(implicit_inclusion$acc, implicit_exclusion$acc))
accuracy_explicit <- as.data.frame(cbind(explicit_inclusion$acc, explicit_exclusion$acc))
write.csv(accuracy_implicit, "accuracy_implicit.csv")
write.csv(accuracy_explicit, "accuracy_explicit.csv")
...
plot(MW_explicit, practice_explicit*1000, pch = 16, col = "#FFB399", ylab = "Practice Effect (ms)", 
     xlab = "Depth of Mindwandering", ylim = c(-400, 400), xlim = c(0, 100))
abline(lm(practice_explicit*1000~MW_explicit), col="#FF7547", lwd = 3)

points(MW_implicit, practice_implict*1000, pch = 16, col = "#A3D2E1")
abline(lm(practice_implict*1000~MW_implicit), col="#0096A3", lwd = 3)

plot(MW_explicit, transfer_explicit*1000, pch = 16, col = "#FFB399", ylab = "Transfer Effect (ms)", 
     xlab = "Depth of Mindwandering", ylim = c(-400, 400), xlim = c(0, 100))
abline(lm(transfer_explicit*1000~MW_explicit), col="#FF7547", lwd = 3)

points(MW_implicit, transfer_implicit*1000, pch = 16, col = "#A3D2E1")
abline(lm(transfer_implicit*1000~MW_implicit), col="#0096A3", lwd = 3)

# run estimates when MW is at 0 and at 100
...
# Reaction Time 
# Main Models in Bayesian 
#summary(lm(transfer_explicit*1000 ~ MW_explicit))
explicittransfer <- cbind(transfer_explicit*1000, MW_explicit)
write.csv(explicittransfer, file= "explicittransfer.csv")
summary(lm(transfer_explicit*1000~ MW_explicit))
# see JASP

#summary(lm(transfer_implicit*1000 ~ MW_implicit))
implicittransfer <- cbind(transfer_implicit*1000, MW_implicit)
write.csv(implicittransfer, file= "implicittransfer.csv")
summary(lm(transfer_implicit*1000 ~ MW_implicit))
# see JASP

#summary(lm(practice_explicit*1000 ~ MW_explicit))
explicitpractice <- cbind(practice_explicit*1000, MW_explicit)
write.csv(explicitpractice, file= "explicitpractice.csv")
# see JASP

#summary(lm(practice_implict*1000 ~ MW_implicit))
implicitpractice <- cbind(practice_implict*1000, MW_implicit)
write.csv(implicitpractice, file= "implicitpractice.csv")
# see JASP

summary(lm(implicit_inclusion$acc~MW_implicit))
summary(lm(explicit_inclusion$acc~MW_explicit))

# model <- lme(RT_Averages ~ Mindwandering*MW_Prior*Block*Prior_RT, random = ~1 | ParticipantID, data = implicit_data)
# model <- lme(RT_Averages ~ Mindwandering*MW_Prior*Block*Prior_RT, random = ~1 | ParticipantID, data = explicit_data)
write.csv(implicit_data, file = "implicit_data.csv")
write.csv(explicit_data, file = "explicit_data.csv")
# see JASP 
...
abline(v = 14, col = "lightgrey", lwd = 4)
lines(ImplicitRT_Plot, type = 'l', lwd = c(SE_Implicit*2), col = '#EBFDFF')
lines(ExplicitRT_Plot, type = 'l', lwd = c(SE_Explicit*2), col = '#FFF0EB')
lines(ImplicitRT_Plot, type = 'l', lwd = 3, col = '#0096A3')
lines(ExplicitRT_Plot, type = 'l', lwd = 3, col = '#FF7547')
